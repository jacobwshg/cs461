...
class GPT2Attention( nn.Module ):
	def __init__(
		self, d_model, heads, max_seq_len,
		...
		# wrap with LoRA
		c_attn = nn.Linear( d_model, 3 * d_model )
		self.c_attn = LoRALinear( c_attn, rank, alpha )

		c_proj = nn.Linear( d_model, d_model )
		self.c_proj = LoRALinear( c_proj, rank, alpha )
...
class GPT2MLP( nn.Module ):
	def __init__(
		self, d_model, d_ff, resid_dropout=0.0,
		rank=8, alpha=16
	):
		super().__init__()

		c_fc = nn.Linear( d_model, d_ff )
		self.c_fc = LoRALinear( c_fc, rank, alpha )
...
class GPT2Block( nn.Module ):
	def __init__(
		...
		self.mlp = GPT2MLP(
			d_model, d_ff, resid_dropout=resid_dropout,
			rank=rank, alpha=alpha
		)
...
def main():
	...
	model = TransformerGPT( 
		...
		dropout=opt.dropout, layer_norm_epsilon=opt.epsilon,
		rank=opt.lora_rank, alpha=opt.lora_alpha
	)
	...
	if opt.task_type == "ZS":
		...
		for epoch in range( opt.epochs ):
			...
			if valid_acc > best_valid_acc:
				...
				save_path = f"aster_lora_obqa.pt"
